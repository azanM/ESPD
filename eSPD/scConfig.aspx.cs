...
namespace eSPD
{
    public partial class scConfig : System.Web.UI.Page
    {
        int HasilEmail = 0;
        protected void Page_Load(object sender, EventArgs e)
        {
            cekJob();
            lblStatusJob.Text = JobScheduler.isStart().ToString();
                ...
                foreach (var item in nrpApproval)
                {
                    ...
                    if (datatoSend.Count != 0 || datatoSend != null)
                    {
                        using (var smtpClient = new SmtpClient())
                        {
                            ...
                            {
                                message.To.Add("espddeveloper@gmail.com");
